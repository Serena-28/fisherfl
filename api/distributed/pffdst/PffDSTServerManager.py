...
class PffDSTServerManager(ServerManager):
    ...
    def mode_convert(self,):
        if current_round > self.args.T_end and hasattr(self, 'mask_fixed') and self.mask_fixed:
        # mask已固定，只做权重聚合
        self.mode = 1
        return self.mode
        ...
        if self.mode == 0:
            ...
            if current_round == self.args.T_end:
            # 在T_end轮次固定mask
            self.mask_fixed = True
            self.mode = 0
