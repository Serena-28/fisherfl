...
class PffDSTAggregator(object):
    ...
    def create_and_store_frozen_masks(self):
    # 获取当前最新的全局模型
        ...
        with torch.no_grad():
            ...
            # 逻辑：我们要训练的是值为 0 的权重，冻结非零的。
            # 所以，当 param.data == 0 时，掩码为 1.0 (可训练)
            # 当 param.data != 0 时，掩码为 0.0 (冻结)
                mask = (param.data == 0).float().to(device)
                trainable_masks[name] = mask

    # 将这个掩码存储起来，或者直接返回
        self.frozen_mask = trainable_masks # 可选，方便调试
